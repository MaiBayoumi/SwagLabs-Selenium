...
public class AllureBinaryManager {
    ...
    public static void downloadAndExtract() {
        try {
            ...
            if (Files.exists(extractionDir)) {
                LogsUtils.info("Allure binaries already exist.");
                return;
            }
            ...
            LogsUtils.info("Allure binaries downloaded and extracted.");
            ...
            LogsUtils.error("Error downloading or extracting binaries" + e.getMessage());
        }
    }
        ...
        try {
            ...
            if (!Files.exists(zipFile)) {
                ...
                try (BufferedInputStream in = new BufferedInputStream(new URI(url).toURL().openStream());
                    ...
                    LogsUtils.error("Invalid URL for Allure download: " + e.getMessage());
                }
            }
            return zipFile;
        } catch (Exception e) {
            LogsUtils.error("Error downloading Allure zip file" + e.getMessage());
            return Paths.get("");
        }
    ...
    private static void extractZip(Path zipPath) {
        try (ZipInputStream zipInputStream = new ZipInputStream(Files.newInputStream(zipPath))) {
            ...
            LogsUtils.error("Error extracting Allure zip file" + e.getMessage());
        }
    }
